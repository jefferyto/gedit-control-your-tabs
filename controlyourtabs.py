...
class ControlYourTabsPlugin(GObject.Object, Gedit.WindowActivatable, PeasGtk.Configurable):
    ...
    def do_create_configure_widget(self):
        ...
        widget = Gtk.Grid(orientation=Gtk.Orientation.VERTICAL)
        button1 = Gtk.RadioButton(label="Tab for switching through most _used tabs", use_underline=True)
        button2 = Gtk.RadioButton(group=button1, label="Tab for switching through _right/left tabs", use_underline=True)
        widget.set_border_width(6)
        #widget.add(Gtk.Label(os.getcwd()))
        widget.add(button1)
        widget.add(button2)
        return widget
    ...
    def _setup(self, tab):
        ...
        if multi:
            self._connect_handlers(multi, ('notebook-added', 'notebook-removed', 'tab-added', 'tab-removed'),
                                   'multi_notebook', notebooks)
            self._multi = multi

            for doc in self.window.get_documents():
                self.on_multi_notebook_notebook_added(multi, Gedit.Tab.get_from_document(doc).get_parent(), notebooks)

            self._connect_handlers(self.window, (
            'tabs-reordered', 'active-tab-changed', 'key-press-event', 'key-release-event', 'focus-out-event'),
                                   'window', notebooks)
